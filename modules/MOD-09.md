...
For **credentials**, a delegated worker inherits neither a secret value nor undocumented credential availability. Its contract may carry only a scoped opaque credential reference and grant lineage; custody, injection, renewal, and revocation remain with the named host and secret owner. A child must not substitute another ambient credential when the declared source is absent. Credential transport proves neither business authority nor permission to broaden the grant.
