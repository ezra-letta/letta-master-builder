...
This original walkthrough illustrates a fresh transfer mission rather than an answer fixture. A fictional archive receives synthetic intake packets from several departments. An offline controller must validate provenance, reconcile packet identity, record at most one review candidate per authorized collection, and prepare an operator disposition when identity, authority, capacity, or retention policy is ambiguous. It never contacts a real archive, receives a credential value, moves live files, or treats these project-owned records as SDK or runtime events.

Before work, an administrator binds the exact package tuple, learner and environment identities, allowed aids, time and retry limits, curriculum lock digest, and withheld perturbations. The controller then keeps four orthogonal state fields rather than one overloaded status:

- **work:** `received → identity-resolved → planned → active → reconciling → completed | stopped`;
- **authority:** `pending → valid → expired | revoked | ambiguous`;
- **execution/effect:** `not-dispatched → submitted-unconfirmed → uncertain → confirmed-success | confirmed-failure | ambiguous-stop`;
- **resource/admission:** `unchecked → admitted | deferred | drained | rejected`.

A transition on one axis never silently rewrites another. Valid authority does not imply available capacity. Admission does not prove dispatch. Dispatch does not prove effect success or work acceptance. Runtime unavailability does not itself revoke business authority, and a confirmed external effect may coexist with revoked authority for every later action.

An intake observation supplies a provisional packet key and department assertion. The synthetic catalog owns canonical collection identity; controller policy owns authorization; a capacity record owns the current bounded reservation decision; and an operator owns ambiguous disposition. The work record carries all four states, canonical IDs, evidence references, budgets, lease generation, stop reason, and acceptance criteria. Two provisional packets may resolve to one collection, so stable operation keys are derived from canonical intent rather than input count.

If a delegated worker requests a credential-backed lookup, delegation transfers only bounded authority to request that operation—not custody of a credential value. Its contract records an opaque credential reference, grant and parent-grant IDs, delegating and acting principals, tenant, purpose, permitted operation and host, validity window, injection owner, and revocation owner. No value enters a prompt, memory, artifact, trace, or child result, and the worker may not substitute ambient credentials or broaden the grant.

Immediately before dispatch, the controller evaluates a project-owned capacity observation: source, scope, freshness or expiry, configured limit, current reservations, requested units, and decision. Missing or stale evidence yields `deferred` or `rejected`, not optimistic admission. A reservation is acquired before dispatch and becomes `drained` or requires reconciliation after completion, cancellation, or uncertainty. Capacity remains separate from readiness, cost budget, lease, authority, account credits, provider quota, and evidence that a run was accepted.

Now two withheld perturbations arrive. First, different provisional keys resolve to one canonical collection; the controller converges on one candidate. Second, department authority is revoked after identity resolution. No remaining budget, capacity, credential, or technical access permits another dispatch. If an earlier request returned a candidate identity but controller persistence failed, the snapshot may correctly read `work: reconciling`, `authority: revoked`, `effect: confirmed-success`, and `admission: drained`. The identity is preserved; retry begins with authoritative lookup. Several matches produce `ambiguous-stop` and operator disposition, never blind creation.

Machine gates could later report exact-package compilation and offline fixture conformance. An independent evaluator would still judge whether the learner preserved the four axes, credential grant lineage, authority loss, duplicate reconciliation, and claim boundaries. Neither machine result nor evaluator judgment supplies runtime validation or production qualification.
